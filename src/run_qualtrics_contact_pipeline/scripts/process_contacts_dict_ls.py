...
def mark_row_processed(
    ...
    """
    Marks a row in the raw sheet as processed by updating the specified status column to True.

    The function looks up the index of the raw_status_col in the worksheet header and
    updates the corresponding cell in the specified row with a True value.

    Args:
        raw_sheet (gspread.Worksheet): The worksheet containing raw contact records.
        row_index (int): The 1-based index of the row to be marked as processed.
        raw_status_col (str, optional): The header name for the column that tracks processing status.
