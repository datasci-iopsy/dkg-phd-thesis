...
# Define target correlation matrix as a pure function
create_target_correlation_matrix <- function() {
    matrix(c(
...
# Pure function to generate demographics
generate_demographics <- function(n_participants) {
    tibble::tibble(
...
# Pure function to create covariance matrix
create_covariance_matrix <- function(correlation_matrix, sds) {
    correlation_matrix * (sds %o% sds)
}

# Pure function to generate base scores
generate_base_scores <- function(n_participants, means, cov_matrix, var_names) {
    mvrnorm(n_participants, mu = means, Sigma = cov_matrix) %>%
        `colnames<-`(var_names)
}

# Pure function to generate item-level data from scale scores
...
# Pure function to generate time-invariant items
...
# Pure function to generate time-varying scores
generate_time_varying_scores <- function(base_scores, n_participants) {
    time_varying_vars <- c(
        "burn_pf_score", "burn_cw_score", "burn_ee_score",
        "nf_comp_score", "nf_auto_score", "nf_rel_score",
        "atcb_score", "turnover_int"
    ...
    (base_scores[, time_varying_vars] + time_variation) %>%
        pmax(1) %>%
        pmin(5)
}

# Pure function to generate time-varying items
...
# Pure function to generate meeting data
...
# Pure function to create timepoint data
create_timepoint_data <- function(
    t,
    n_participants,
    time_invariant_items,
    time_varying_items,
    meeting_data,
    demographics) {
    tibble::tibble(
...
main_pipeline <- function() {
    ...
    target_corr_matrix <- create_target_correlation_matrix()
    ...
    cov_matrix <- create_covariance_matrix(target_corr_matrix, sds)
    base_scores <- generate_base_scores(
        n_participants, means, cov_matrix,
        colnames(target_corr_matrix)
    )
    time_invariant_items <- generate_time_invariant_items(base_scores)

    # Generate data for each timepoint using functional approach
    full_data <- map_dfr(1:n_timepoints, ~ {
        time_varying_scores <- generate_time_varying_scores(base_scores, n_participants)
...
full_data %>%
    ...
    write_csv(here("r-work", "claude-syn-data-raw-v2.csv"))
