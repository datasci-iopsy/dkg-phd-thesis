...
def main(request):
    ...
    config_path = "configs"
    config = common_utils.load_configs(config_path=config_path, use_box=True)
    # print(config)

    client = auth_gspread_via_secret_manager(config=config)
    print(client.get_file_drive_metadata(id=config.params.gspread.spreadsheet_id))
    # print(client)

    # raw_sheet, clean_sheet = get_raw_and_clean_sheets(client=client, config=config)
    # print(raw_sheet.get_all_records(), clean_sheet.get_all_records)
