...
gcp/
  cloud_run_functions/
    run_qualtrics_scheduling/     # Intake webhook: validates + writes to BQ
      configs/
        gcp_config.yaml           # GCP project, BigQuery, Pub/Sub topic
    ...
    run_intake_confirmation/      # SMS confirmation: Pub/Sub triggered
      configs/
        gcp_config.yaml           # GCP project, BigQuery (no Qualtrics/Pub/Sub)
      main.py                     # Cloud Function entry point
      requirements.txt            # Generated by Poetry at deploy time
  shared/
    utils/
...
The pipeline processes completed survey responses in real time through two Cloud Run functions connected by Pub/Sub.
...
The confirmation function ([`run_intake_confirmation/main.py`](cloud_run_functions/run_intake_confirmation/main.py)) is triggered by Pub/Sub via an Eventarc push subscription. It decodes the CloudEvent message, checks BigQuery for idempotency (`_processed` flag), sends an SMS via Twilio confirming the participant's follow-up schedule, and updates `_processed = TRUE`. If SMS fails, the function raises so Pub/Sub retries with exponential backoff. Malformed messages are acknowledged to prevent infinite retries.

## CLI tools

Four scripts live in `gcp/deploy/` and are run from the project root. All four use argparse, validate their configuration with Pydantic, and support `--help`.
...
`manage_pubsub.py` manages the Pub/Sub topic that connects the scheduling function (publisher) to the confirmation function (subscriber). The script only manages the topic itself. The Eventarc push subscription is created automatically when the consuming function is deployed with a `topic` trigger via `manage_functions.py`.
...
**[`run_intake_confirmation/configs/gcp_config.yaml`](cloud_run_functions/run_intake_confirmation/configs/gcp_config.yaml)**: GCP project ID, compute region, and BigQuery references. Does not include Qualtrics or Pub/Sub sections since this function receives messages via Eventarc push subscription rather than publishing them.
...
The intake confirmation function uses Twilio for SMS delivery. Credentials are stored as a single JSON secret in Secret Manager and injected as the `TWILIO_CONFIG` environment variable at deploy time.
...
Replace the placeholder values with your actual Twilio credentials. The `from_number` must be an E.164-formatted Twilio phone number you own.
...
- `fn-qualtrics-scheduling`: Dependencies specific to the scheduling function (Flask, functions-framework, google-cloud-bigquery, google-cloud-pubsub).
- `fn-intake-confirmation`: Dependencies specific to the confirmation function (functions-framework, google-cloud-bigquery, twilio, cloudevents).
