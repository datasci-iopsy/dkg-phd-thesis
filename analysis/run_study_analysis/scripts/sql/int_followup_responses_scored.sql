...
with
    source as (
        -- join supplement and rank within (intake_response_id, survey_id) to deduplicate
        select
            f.*,
            s.meetings_time as supplement_meetings_time,
            row_number() over (
                partition by f.intake_response_id, f.survey_id order by f._created_at asc
            ) as _rn
        from
            `dkg-phd-thesis.qualtrics.stg_followup_responses` as f
            left join `dkg-phd-thesis.qualtrics.meeting_time_supplement` as s
                on f.response_id = s.response_id
    ...
    transformed as (
        select
            ...
            case
                when (survey_id = 'SV_5nV942MJGubDmqq' and attention_check = 'Once')
                    or (survey_id = 'SV_eRKl4lgMZDAurT8' and attention_check = 'Strongly agree')
                    or (survey_id = 'SV_6J3svun1r97AAHc' and attention_check = 'Strongly disagree')
                    then true
                else false
