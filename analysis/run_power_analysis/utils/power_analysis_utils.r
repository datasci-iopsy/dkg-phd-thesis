...
# tests for L1 direct, L2 direct, and cross-level interaction effects.
# ---------------------------------------------------------------------------
...
#' Power analysis for two-level (multilevel) models
#'
#' Constructs a population model from standardized parameters following
#' Arend & Schafer (2019), then runs Monte Carlo power simulations via
#' simr::powerSim() with Kenward-Roger tests for each fixed effect.
#'
...
#' @param xlvl_effect_std Standardized cross-level interaction effect
#'   (default: 0.50)
...
#' @return If return_df is TRUE, a tibble with power results and parameters.
#'   Otherwise, a power_analysis_2level list object.
#'
power_analysis_two_level <- function(
  ...
  rand_seed = 1234
    ...
    # Step 1: Calculate variance components (Equations 10-11)
    ...
    # Conduct power analyses for each fixed effect
    ...
    # Extended summary with full parameter context (used by grid runner)
    ...
    # Compact summary for interactive use
...
#' @param extended Logical, return extended summary with parameters
#'   (default: TRUE)
...
power_to_long <- function(power_results) {
    ...
    long_df <- df |>
        dplyr::select(Effect, Power, Lower_CI, Upper_CI) |>
        tidyr::pivot_longer(
...
#' Print method for power_analysis_2level objects
print.power_analysis_2level <- function(x, ...) {
    ...
    cat("\nUse extract_power_data() to get data frame for further analysis\n")
    invisible(x)
}


#' Summary method for power_analysis_2level objects
summary.power_analysis_2level <- function(object, extended = TRUE, ...) {
    extract_power_data(object, extended = extended)
}
